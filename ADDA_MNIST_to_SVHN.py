...
wandb.init(project="ADDA-MNIST-to-SVHN")
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 64, 5),  # Update input channels to 3
            nn.BatchNorm2d(64),
...
def main():
    ...
    # Load SVHN dataset as target domain
    target_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))
    ])

    # Update SVHN dataset loading to use datasets.SVHN and set download to False
    target_dataset = datasets.SVHN(root='./data', split='train', download=False, transform=target_transform)
    ...
    for epoch in range(num_epochs):
        ...
        for source_data, target_data in train_bar:
            ...
            # Padding and converting MNIST images to RGB
            source_images = pad(source_images, padding=(2, 2, 2, 2), fill=0, padding_mode='constant')
            source_images = source_images.expand(-1, 3, -1, -1)
        ...
        with torch.no_grad():
            for test_data in tqdm(target_dataloader, total=len(target_dataloader)):
                ...
                # Padding and converting SVHN images to RGB
                test_images = test_images.expand(-1, 3, -1, -1)
