...
def main():
    ...
    wandb.init(entity="hails",
               ...
               name="[Prunus]SC_specialization_in_lr:" + str(args.lr)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (svhn_data, cifar_data1) in enumerate(zip(svhn_loader, cifar_loader1)):
            ...
            cifar_out_part1, cifar_domain_out1, cifar_part_idx1, cifar_part_gumbel = model(cifar_images1, alpha=lambda_p, tau=args.tau, return_partition_prob=True)
            ...
            if i % 1 == 0:
                ...
                # cifar_counts2 = torch.bincount(cifar_part_idx2, minlength=args.num_partition)
                # print(f"MNIST : {mnist_counts.cpu().numpy()} / SVHN  : {svhn_counts.cpu().numpy()} / CIFAR : {(cifar_counts1 + cifar_counts2).cpu().numpy()}")
                print(f"SVHN  : {svhn_counts.cpu().numpy()} / CIFAR : {cifar_counts1.cpu().numpy()}")
            ...
            all_probs = torch.cat((svhn_part_gumbel, cifar_part_gumbel), dim=0)
            avg_prob_global = torch.mean(all_probs, dim=0) # [M] 크기
            ...
            # 최종 전문성 Loss
            loss_specialization = loss_specialization_svhn + loss_specialization_cifar

            # label_loss = svhn_label_loss + cifar_label_loss1 - args.reg_alpha * label_loss + (reg_svhn + reg_cifar) * args.reg_beta

            label_loss = svhn_label_loss + cifar_label_loss1 +  args.reg_alpha * loss_specialization + args.reg_beta * loss_diversity
            ...
            total_cifar_loss += cifar_label_loss1.item()

            total_domain_loss += domain_loss.item()
            # total_mnist_domain_loss += mnist_domain_loss.item()
            total_svhn_domain_loss += svhn_domain_loss.item()
            # total_cifar_domain_loss += (cifar_domain_loss1.item() + cifar_domain_loss2.item())
            total_cifar_domain_loss += cifar_domain_loss1.item()
