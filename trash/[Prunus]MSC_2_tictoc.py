...
def main():
    ...
    wandb.init(entity="hails",
               ...
               name="[Prunus]TickTock_SC_lr:" + str(args.lr)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (svhn_data, cifar_data1) in enumerate(zip(svhn_loader, cifar_loader1)):
            ...
            if i % 1 == 0:
                ...
                print(f"SVHN  : {svhn_counts.cpu().numpy()}")
            ...
            if i % 1 == 0:
                ...
                # cifar_counts2 = torch.bincount(cifar_part_idx2, minlength=args.num_partition)
                # print(f"MNIST : {mnist_counts.cpu().numpy()} / SVHN  : {svhn_counts.cpu().numpy()} / CIFAR : {(cifar_counts1 + cifar_counts2).cpu().numpy()}")
                print(f"CIFAR : {cifar_counts1.cpu().numpy()}")
                print(f"Switcher Weight Mean: {model.partition_switcher.weight.data.mean():.10f}, Bias Mean: {model.partition_switcher.bias.data.mean():.10f}")
